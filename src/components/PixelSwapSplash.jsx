...
const PATTERNS = {
  ...
  'center out': (x, y) => Math.hypot(x - 0.5, y - 0.5) / Math.SQRT1_2,
  'center-out': (x, y) => Math.hypot(x - 0.5, y - 0.5) / Math.SQRT1_2,
...
const buildKeyframes = ({ ease, startScale, endScale, spin, fade, reverse }) => {
  const window = [];
  const content = [];
  ...
  for (let step = 0; step <= KEYFRAME_STEPS; step += 1) {
    ...
    const opacity = reverse
      ? (fade ? Math.max(0, 1 - eased * 1.6) : 1)
      : (fade ? Math.min(1, eased * 1.6) : 1);
...
function PixelSwapSplash({
  ...
  const config = { duration, pixelDuration, pixelSpin, pixelScale, pixelRadius, fade, easing, onComplete, animationDirection };
  ...
  useEffect(() => {
    ...
    const isOut = settings.animationDirection === 'out';
    const sourceIndex = isOut ? (to ? 0 : 1) : (to ? 1 : 0);
  ...
  const renderLayer = (content, index) => {
    ...
    return (
      <div
        ...
        data-visible={isShown && !(transition && index === hideIndex)}
...
export default PixelSwapSplash;
